function [ Skylight_Relative_Intensity ] = Simu_Sky_Intensity_CIE( Sun_azimuth_rad,...
    Sun_elevation_rad, Sky_Particles_azimuth_rad,...
    Sky_Particles_elevation_rad, CIE_Sky_number )
...
CIE_coef = [4.0 , -0.7 , 0 , -1 , 0;
    ...
    x_sun=cos(Sun_elevation_rad)*cos(Sun_azimuth_rad);
    y_sun=cos(Sun_elevation_rad)*sin(Sun_azimuth_rad);
    z_sun=sin(Sun_elevation_rad);
    sun_zenith_angle_rad=pi/2-Sun_elevation_rad;
    X_sky_particles=cos(Sky_Particles_elevation_rad).*cos(Sky_Particles_azimuth_rad);
    Y_sky_particles=cos(Sky_Particles_elevation_rad).*sin(Sky_Particles_azimuth_rad);
    Z_sky_particles=sin(Sky_Particles_elevation_rad);
    Sky_particles_zenith_angle_rad=pi/2-Sky_Particles_elevation_rad;
    
    a_coef=CIE_coef(CIE_Sky_number,1);
    b_coef=CIE_coef(CIE_Sky_number,2);
    c_coef=CIE_coef(CIE_Sky_number,3);
    d_coef=CIE_coef(CIE_Sky_number,4);
    e_coef=CIE_coef(CIE_Sky_number,5);
    
    cos_scatt_angle = (x_sun*X_sky_particles + y_sun*Y_sky_particles...
        + z_sun*Z_sky_particles);
    cos_square_scatt_angle = cos_scatt_angle.^2;
    scatt_angle_rad=acos(cos_scatt_angle);
    ...
    Phy_CIE_ratio=(1+a_coef*exp(b_coef./cos(Sky_particles_zenith_angle_rad)))...
        /(1+a_coef*exp(b_coef));
    
    f_CIE_ratio= (1+c_coef*(exp(d_coef*scatt_angle_rad)-exp(d_coef*pi/2))+...
        e_coef*cos_square_scatt_angle)...
        /...
        (1+c_coef*(exp(d_coef*sun_zenith_angle_rad)-exp(d_coef*pi/2))+...
        e_coef*(cos(sun_zenith_angle_rad)^2));
    
    %end of calculation
    Skylight_Relative_Intensity=Phy_CIE_ratio.*f_CIE_ratio;
